class CheckRemoteChecksum < Scout::Plugin
  def build_report
    begin
      ...
      if exitcode != 0
        alert(:subject => "Checksum failed for file #{remote_md5_path}", :body => output)
      end
      return exitcode
    rescue Exception => e
      error(:subject => 'Error running Check Remote Checksum plugin', :body => e)
      return -1
    end
  end
end